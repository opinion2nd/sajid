package dev.opinion2nd.antifreecam.util;
 ...
 * surfacing).
 *
 * <p>Paper runs on Mojang-mapped NMS from 1.20.5+, so we resolve everything by
 * its Mojang name via reflection. Everything is cached after the first call and
 * guarded — if the server internals ever change, masking still works, only the
 * progressive reveal is skipped (and a single warning is logged).
 */
public final class ChunkResender {
    ...
    private synchronized void init(Player sample) throws ReflectiveOperationException {
        ...
        // ServerPlayer extends Player extends LivingEntity extends Entity.
    ...
    /** Re-send the given chunk to the player. Safe to call on the main thread. */
