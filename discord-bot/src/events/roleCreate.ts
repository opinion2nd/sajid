...
export async function execute(role: Role) {
  ...
  const log = await guild.fetchAuditLogs({ type: AuditLogEvent.RoleCreate, limit: 1 }).catch(() => null);
  ...
  if (exceeded) {
    await punishNukeExecutor(guild, entry.executor.id, `Created ${config.nuke_threshold}+ roles in ${config.nuke_window_seconds}s`);
  }
}
