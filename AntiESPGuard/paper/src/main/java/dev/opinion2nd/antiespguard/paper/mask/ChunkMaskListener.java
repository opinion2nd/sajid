 ...
 * {@link MaskService} says must be masked. This is the core anti-freecam /
 * anti-ESP defence: surface players never receive real underground geometry.
 */
public final class ChunkMaskListener extends PacketListenerAbstract {
    ...
    public void onPacketSend(PacketSendEvent event) {
        ...
        int maskId = SpigotConversionUtil
                .fromBukkitBlockData(cfg.maskMaterial().createBlockData())
                .getGlobalId();
        int minY = player.getWorld().getMinHeight();
        int hideBelowY = cfg.raw().hideBelowY;

        boolean changed = maskColumn(column, minY, hideBelowY, maskId,
                cfg.raw().skipMaskIfAlreadyAir);
        if (changed) {
